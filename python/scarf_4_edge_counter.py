...
class scarf_4_edge_counter:
	...
	def get_count_val(self, counter_num=1, series_num=1):
		if (counter_num < 1 or counter_num > 4):
			print "Must specify a counter_num between 1 to 4"
		elif (series_num < 1 or series_num > 3):
			print "Must specify a series_num of 1, 2 or 3"
		else:
	...
	def read_all_regmap(self):
		read_list = self.scarf_slave.read_list(addr=0x00, num_bytes=50)
